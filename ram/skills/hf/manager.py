...
Key features:
  ...
  - Registration of downloaded models into Ram's LLM router so they are
    automatically used for "private" and "cheap" tasks

Typical usage::

    from ram.skills.hf.manager import download_model, list_installed, run_model
...
from ram.skills.hf.catalog import HFModel, get as catalog_get, ALL_MODELS

# ── Constants ─────────────────────────────────────────────────────────────

# Where downloaded HF models are cached on disk
_HF_CACHE = Path(os.environ.get("HF_HOME", str(Path.home() / ".cache" / "huggingface")))
_INSTALLED_DB = Path(settings.ram_data_dir) / "hf_installed.json"
...
def download_model(
    ...
    """Download a model from HuggingFace and make it available to Ram.
    ...
    4. Registers the model in Ram's LLM router for "private" tasks.
...
def _register_in_router(model: HFModel, via: str) -> None:
    ...
    """
    try:
        from ram.core.llm import _CATALOG, ProviderModel
