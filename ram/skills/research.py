"""Deep research skill — Ram researches anything like an expert analyst.
...
The research skill is the key differentiator between Ram and a chatbot —
...
from ram.core.llm import llm_chat, llm_classify
...
def _web_search(query: str, max_results: int = 5) -> str:
    ...
    if settings.perplexity_api_key:
        try:
            from ram.core.llm import _call_perplexity
...
def research_person(name: str, company: str = "", context: str = "") -> str:
    ...
    local_notes = ""
    try:
        with __import__("ram.core.memory", fromlist=["db"]).db() as s:
            from ram.core import contexts
