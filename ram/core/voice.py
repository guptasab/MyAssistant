...
STT (transcription):
  ...
  3. Local Whisper   — fully offline via HuggingFace transformers
                       (requires RAM_LOCAL_WHISPER=true)
...
Streaming TTS:
  ...
  This powers the real-time voice conversation feature — Ram starts speaking
...
def transcribe(audio_bytes: bytes, mime: str = "audio/webm") -> str:
    ...
    if getattr(settings, "ram_local_whisper", False):
    ...
    logger.warning(
        ...
        "Set GROQ_API_KEY, OPENAI_API_KEY, or RAM_LOCAL_WHISPER=true."
    )
    return ""
...
def synthesize(text: str, voice_id: str | None = None) -> Path | None:
    ...
    """
    out_dir = settings.ram_data_dir / "audio"
