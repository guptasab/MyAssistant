...
from ram.core import memory, registry
...
class Agent:
    def __init__(self):
        ...
        try:
            ...
            pass
        self.model = settings.ram_model_main
    ...
    async def _check_pending_confirmation(self, user_id: str, user_text: str) -> AgentReply | None:
        ...
        from ram.core import confirm as conf
        ...
        # Push to undo stack
        try:
            from ram.core import undo as undo_mod
    ...
    async def handle(self, user_id: str, user_text: str) -> AgentReply:
        ...
        if not any(list_providers().values()):
            return AgentReply(text=(
                "(Ram has no LLM provider configured — set ANTHROPIC_API_KEY "
