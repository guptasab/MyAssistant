...
async def _poll_school_inbox(notify: NotifyFn) -> None:
    try:
        from ram.skills.school_inbox import scan_school_inbox, _unsurfaced_actions, _mark_surfaced
    except Exception as e:
...
async def _carpool_nudges(notify: NotifyFn) -> None:
    ...
    try:
        from ram.skills.carpool import carpools_for_today
    except Exception:
        return
    f = fam.get_or_create_default_family()
...
async def _deadman_job(notify: NotifyFn) -> None:
    try:
        from ram.skills.safety import deadman_check_and_alert
...
async def _price_check_job(notify: NotifyFn) -> None:
    try:
        from ram.skills.commerce import check_prices
