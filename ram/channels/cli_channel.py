...
class CLIChannel(Channel):
    ...
    async def start(self) -> None:
        self.console.print("[bold cyan]Ram[/bold cyan] is ready. Type 'exit' to quit.\n")
        ...
        while True:
            ...
            self.console.print(f"[bold magenta]Ram[/bold magenta]: {reply.text}\n")
            if reply.actions_taken:
                self.console.print(f"[dim]actions: {', '.join(reply.actions_taken)}[/dim]\n")

    async def send(self, user_id: str, text: str) -> None:
        self.console.print(f"\n[bold yellow]Ram (push)[/bold yellow]: {text}\n")
