...
Run separately:  python -m myassistant.tools.browser_worker
...
PROFILE = settings.myassistant_data_dir / "browser_profile"
...
async def _run():
    ...
    async with async_playwright() as pw:
        ...
        while True:
            for kind, q in [("gvoice", settings.myassistant_data_dir / "gvoice_queue"),
                            ("order", settings.myassistant_data_dir / "order_queue")]:
                if not q.exists():
                    continue
