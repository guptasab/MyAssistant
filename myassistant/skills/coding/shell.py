...
allowlist via MYASSISTANT_SHELL_ALLOWLIST (comma-separated prefixes) in .env.
"""
...
_TIMEOUT = int(getattr(settings, "myassistant_shell_timeout", 60))
...
def _get_allowlist() -> list[str]:
    extra = getattr(settings, "myassistant_shell_allowlist", "")
    if extra:
...
def exec_shell(command: str, repo: str = "", timeout: int = 60, dry_run: bool = False) -> str:
    if not _is_allowed(command):
        return (f"BLOCKED: '{command}' is not in the shell allowlist.\n"
                ...
                f"Add more via MYASSISTANT_SHELL_ALLOWLIST env var.")
    if dry_run:
