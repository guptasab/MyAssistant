"""debug_prod — composite "Hey MyAssistant, debug the last call in prod" skill.
...
def debug_prod(service: str = "", minutes: int = 30,
    ...
    if settings.aws_access_key_id and settings.aws_secret_access_key:
        try:
            from myassistant.skills.aws.cloudwatch import cw_list_groups, cw_filter_logs, cw_error_summary
            from myassistant.skills.aws.lambda_skill import lambda_list, lambda_find_errors
    ...
    if settings.aws_access_key_id and settings.aws_secret_access_key:
        try:
            from myassistant.skills.aws.xray import xray_traces
...
{context_for_llm[:3500]}"""
    ...
    if open_issue and settings.github_token:
        try:
            from myassistant.skills.github_skill import github_create_issue
