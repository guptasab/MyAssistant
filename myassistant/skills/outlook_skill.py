...
Each email account is stored in myassistant.core.accounts and is accessed via
...
def _get_client(email: str | None = None):
    ...
    """
    from myassistant.core.accounts import list_accounts, graph_client_for
    if email:
...
def outlook_create_event(title: str, start: str, end: str,
    ...
    payload: dict = {
        "subject": title,
        "start": {"dateTime": start, "timeZone": settings.myassistant_timezone},
        "end":   {"dateTime": end,   "timeZone": settings.myassistant_timezone},
    }
    if location:
