...
First-run flow: agent will instruct the user to run `python -m myassistant.tools.google_auth`.
"""
...
_TOKEN_PATH = settings.myassistant_data_dir / "google_token.json"
...
def _list_range(days_offset: int) -> str:
    svc = _service()
    if not svc:
        return "ERROR: Google Calendar not connected. Run `python -m myassistant.tools.google_auth`."
...
def schedule_event(title: str, start_iso: str, end_iso: str, attendees_emails: list = None,
    ...
    body = {
        ...
        "start": {"dateTime": start_iso, "timeZone": settings.myassistant_timezone},
        "end": {"dateTime": end_iso, "timeZone": settings.myassistant_timezone},
    }
    if attendees_emails:
