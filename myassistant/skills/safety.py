...
def emergency_packet() -> str:
    ...
    try:
        from myassistant.skills.medications import Medication
        with db() as s:
...
def deadman_status() -> str:
    from myassistant.core.memory import Message
    with db() as s:
...
def deadman_check_and_alert() -> bool:
    ...
    if not settings.ollie_deadman_contact:
        return False
    try:
        from myassistant.channels.sms_channel import send_sms
        send_sms(settings.ollie_deadman_contact,
                 f"Ollie deadman: no activity from {settings.myassistant_owner_name} in {age_h:.0f}h.")
        return True
    except Exception:
        return False
