...
All endpoints require Authorization: Bearer <MYASSISTANT_HTTP_TOKEN>.
"""
...
def _check_auth(authorization: str | None) -> None:
    if not settings.myassistant_http_token:
        return  # no token configured -> open (dev only)
    if not authorization or authorization != f"Bearer {settings.myassistant_http_token}":
...
class HTTPChannel(Channel):
    ...
    def _build_app(self) -> FastAPI:
        ...
        async def audio_file(name: str):
            p = settings.myassistant_data_dir / "audio" / name
            if not p.exists():
        ...
        async def location_update(payload: dict, authorization: str | None = Header(None)):
            ...
            _check_auth(authorization)
            try:
                from myassistant.skills.location_skill import update_location
        ...
        async def stream(ws: WebSocket):
            ...
            if settings.myassistant_http_token and token != settings.myassistant_http_token:
        ...
        try:
            from myassistant.channels.admin_ui import build_admin_router, build_shortcuts_router
        ...
        try:
            from myassistant.channels.voice_channel import build_voice_router
        ...
        try:
            from myassistant.skills.voice_assistants import build_voice_assistants_router
        ...
        try:
            from myassistant.core.mcp_server import build_mcp_fastapi_router
        ...
        async def canvas_ws(ws: WebSocket):
            ...
            """
            await ws.accept()
            from myassistant.core import canvas_state as cv
            import json
        ...
        async def twilio_sms(request: Request):
            ...
            tag = from_num
            try:
                from myassistant.core.family import find_member_by_phone
    ...
    async def start(self) -> None:
        cfg = uvicorn.Config(
            self._app, host=settings.myassistant_http_host, port=settings.myassistant_http_port,
            log_level="info", lifespan="on",
        )
        self._server = uvicorn.Server(cfg)
        asyncio.create_task(self._server.serve())
        logger.info(f"HTTP channel on http://{settings.myassistant_http_host}:{settings.myassistant_http_port}")
