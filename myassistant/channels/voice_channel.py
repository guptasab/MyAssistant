"""Real-time voice conversation channel — the "Gemini app" experience for MyAssistant.
...
1. **WebSocket** ``/voice/realtime``
   JSON protocol::
     ...
     Server  → {"type": "transcript",  "text": "..."}       # what MyAssistant heard
...
matching the Gemini-app UX where MyAssistant starts speaking before it finishes thinking.

Architecture::

    browser/tray → WebSocket → VoiceSession
                                    ├── STT     (Groq / OpenAI / local)
                                    ├── Agent   (myassistant.core.agent)
...
class VoiceSession:
    ...
    Protocol overview::
        ...
        2. Session transcribes audio via :mod:`myassistant.core.voice`.
...
def build_voice_router(agent_handle: Callable) -> APIRouter:
    ...
    async def voice_realtime(ws: WebSocket):
        ...
        Query params:
            token   — Bearer token (matches MYASSISTANT_HTTP_TOKEN if set)
