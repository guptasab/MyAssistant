"""Full-featured admin + configuration web UI for MyAssistant.
...
_FIELDS: list[dict] = [
    ...
    {"key": "myassistant_owner_name",         "label": "Your Name",                "group": "profile",    "type": "text"},
    {"key": "myassistant_timezone",           "label": "Timezone (e.g. US/Pacific)","group": "profile",   "type": "text"},
    {"key": "myassistant_data_dir",           "label": "Data Directory",           "group": "profile",    "type": "text"},
    ...
    {"key": "myassistant_model_main",         "label": "Main Model ID",            "group": "profile",    "type": "text",  "help": "e.g. claude-opus-4-7"},
    {"key": "myassistant_model_fast",         "label": "Fast Model ID",            "group": "profile",    "type": "text"},
    # ── AI Providers ─────────────────────────────────────────────────────
    ...
    {"key": "myassistant_http_host",          "label": "HTTP Host",                "group": "channels",   "type": "text"},
    {"key": "myassistant_http_port",          "label": "HTTP Port",                "group": "channels",   "type": "text"},
    {"key": "myassistant_http_token",         "label": "HTTP Bearer Token",        "group": "channels",   "type": "secret"},
    # ── Safety & Security ────────────────────────────────────────────────
    ...
    # ── Local Resources ──────────────────────────────────────────────────
    {"key": "myassistant_repo_workspace",     "label": "Repo Workspace Path",      "group": "resources",  "type": "text",  "help": "Local path where repos are cloned"},
    {"key": "myassistant_shell_allowlist",    "label": "Shell Allowlist (extra)",  "group": "resources",  "type": "text",  "help": "Comma-separated command prefixes"},
    {"key": "myassistant_shell_timeout",      "label": "Shell Timeout (seconds)",  "group": "resources",  "type": "text"},
...
const ADMIN_TOKEN = localStorage.getItem('myassistant_admin_token') || '';
...
async function loadSetupSteps() {{
  ...
  const briefingOk   = cfg.myassistant_briefing_time && cfg.myassistant_briefing_time !== '07:30';
  const httpOk       = cfg.myassistant_http_token;
...
async function loadConfig() {{
  try {{
    ...
    document.getElementById('hdr-status').textContent =
      `Owner: ${{r.myassistant_owner_name || '?'}} · ${{r.myassistant_timezone || 'UTC'}}`;
  }} catch(e) {{
...
async function apiFetch(url, method='GET', body=null) {{
  ...
  const tok = localStorage.getItem('myassistant_admin_token');
  if (tok) opts.headers['Authorization'] = 'Bearer ' + tok;
  if (body) opts.body = JSON.stringify(body);
  const r = await fetch(url, opts);
  if (r.status === 401) {{
    const pw = prompt('Admin password:');
    if (pw) {{ localStorage.setItem('myassistant_admin_token', pw); }}
...
def build_admin_router() -> APIRouter:
    ...
    def skills_json(authorization: str | None = Header(None)):
        _auth(authorization)
        from myassistant.core import registry
    ...
    def backup_now(authorization: str | None = Header(None)):
        _auth(authorization)
        from myassistant.core.backup import export_zip
    ...
    def doctor_check(authorization: str | None = Header(None)):
        ...
        _auth(authorization)
        from myassistant.core.doctor import run_doctor
        results = run_doctor()
        return {
    ...
    def memory_fact_delete(key: str, authorization: str | None = Header(None)):
        ...
        _auth(authorization)
        from myassistant.core import memory as mem
    ...
    def memory_wipe(authorization: str | None = Header(None)):
        ...
        _auth(authorization)
        from myassistant.core.memory import clear_history
        clear_history("cli")
        return {"ok": True}
    ...
    def memory_export(authorization: str | None = Header(None)):
        ...
        _auth(authorization)
        import json, time
        from myassistant.core.memory import export_memory
...
def build_shortcuts_router(handle) -> APIRouter:
    ...
    async def shortcut_ask(request: Request,
        ...
        if settings.myassistant_http_token and authorization != f"Bearer {settings.myassistant_http_token}":
