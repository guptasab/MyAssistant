...
class Settings(BaseSettings):
    ...
    myassistant_owner_name: str = Field("Owner", env=["MYASSISTANT_OWNER_NAME", "RAM_OWNER_NAME"])
    myassistant_timezone: str = Field("UTC", env=["MYASSISTANT_TIMEZONE", "RAM_TIMEZONE"])
    myassistant_data_dir: Path = Field(Path("./data"), env=["MYASSISTANT_DATA_DIR", "RAM_DATA_DIR"])
    ...
    myassistant_model_main: str = Field("claude-opus-4-7", env=["MYASSISTANT_MODEL_MAIN", "RAM_MODEL_MAIN"])
    myassistant_model_fast: str = Field("claude-haiku-4-5-20251001", env=["MYASSISTANT_MODEL_FAST", "RAM_MODEL_FAST"])
    ...
    myassistant_http_host: str = Field("0.0.0.0", env=["MYASSISTANT_HTTP_HOST", "RAM_HTTP_HOST"])
    myassistant_http_port: int = Field(8765, env=["MYASSISTANT_HTTP_PORT", "RAM_HTTP_PORT"])
    myassistant_http_token: str = Field("", env=["MYASSISTANT_HTTP_TOKEN", "RAM_HTTP_TOKEN"])
    ...
    # Coding / DevOps
    myassistant_repo_workspace: str = Field("", env=["MYASSISTANT_REPO_WORKSPACE", "RAM_REPO_WORKSPACE"])                      # local path for cloned repos
    myassistant_shell_allowlist: str = Field("", env=["MYASSISTANT_SHELL_ALLOWLIST", "RAM_SHELL_ALLOWLIST"])                     # extra comma-separated allowed cmd prefixes
    myassistant_shell_timeout: int = Field(60, env=["MYASSISTANT_SHELL_TIMEOUT", "RAM_SHELL_TIMEOUT"])                       # max seconds for exec_shell
    github_default_repo: str = ""                     # "owner/repo" for default issue creation

    # Voice
    myassistant_local_whisper: bool = Field(False, env=["MYASSISTANT_LOCAL_WHISPER", "RAM_LOCAL_WHISPER"])                   # use local Whisper model for STT (offline)
    ...
    def ensure_dirs(self) -> None:
        self.myassistant_data_dir.mkdir(parents=True, exist_ok=True)
        (self.myassistant_data_dir / "logs").mkdir(exist_ok=True)
        (self.myassistant_data_dir / "audio").mkdir(exist_ok=True)
