"""Proactive intelligence engine — MyAssistant anticipates needs before you ask.

This module powers the things that make MyAssistant feel like a great human assistant
...
The scheduler in ``myassistant.core.proactive`` calls these functions periodically.
The agent can also call them directly when the user asks for a status update.

All suggestions go through ``myassistant.core.suggestions`` so duplicates are suppressed
...
def build_meeting_brief(event_title: str, attendees: list[str]) -> str:
    ...
    """
    from myassistant.skills.meetings import prep_for_meeting
...
def proactive_intelligence_report() -> str:
    ...
    # Budget anomalies
    try:
        from myassistant.core.anomaly import detect_anomalies
