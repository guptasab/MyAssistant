...
def run_doctor() -> list[CheckResult]:
    ...
    http_token = getattr(settings, "myassistant_http_token", "")
    results.append(_check(
        ...
        "Set MYASSISTANT_HTTP_TOKEN=<random-string> in .env",
    ))
    ...
    # ── Data & Backup ─────────────────────────────────────────────────────
    data_dir = getattr(settings, "myassistant_data_dir", Path("./data"))
    results.append(_check(
