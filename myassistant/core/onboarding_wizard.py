...
def _test_llm(env: dict[str, str]) -> bool:
    ...
    try:
        from myassistant.core.llm import llm_chat, list_providers
...
def _setup_email(env: dict[str, str]) -> bool:
    ...
    try:
        ...
        token_path = settings.myassistant_data_dir / "google_token.json"
    except Exception:
    ...
    if _confirm("Open Google login now?"):
        try:
            from myassistant.skills.gmail_skill import _get_gmail_service
            _get_gmail_service()
...
def _setup_briefing(env: dict[str, str]) -> bool:
    ...
    env["MYASSISTANT_BRIEFING_TIME"] = time_str

    name = _ask("What's your first name? (Squire will use this)")
    if name:
        env["MYASSISTANT_OWNER_NAME"] = name
...
def _setup_http(env: dict[str, str]) -> bool:
    ...
    existing = env.get("MYASSISTANT_HTTP_TOKEN", "")
    if existing:
    ...
    if _confirm("Set a password for the web interface?", default=True):
        ...
        if token:
            env["MYASSISTANT_HTTP_TOKEN"] = token
...
def _final_test(env: dict[str, str]) -> None:
    ...
    try:
        ...
        async def _test():
            name = env.get("MYASSISTANT_OWNER_NAME", "")
