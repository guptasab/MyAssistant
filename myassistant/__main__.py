...
from myassistant.core import registry, scheduler
from myassistant.core.agent import get_agent
from myassistant.core.config import settings
from myassistant.skills.notify import register_channel
...
async def _run(channels_arg: str, sandbox: bool = False) -> None:
    ...
    logger.add(settings.myassistant_data_dir / "logs" / "squire.log", rotation="10 MB", retention=5, level="DEBUG")
    ...
    if not active:
        ...
        from myassistant.core.onboarding_wizard import needs_onboard
        if needs_onboard():
            logger.warning(
                "\n"
                "  ╔══════════════════════════════════════════════════╗\n"
    ...
    # Load plugins
    try:
        from myassistant.core.plugin_registry import load_all_plugins
    ...
    from myassistant.channels.cli_channel import CLIChannel
    from myassistant.channels.discord_channel import DiscordChannel
    from myassistant.channels.telegram_channel import TelegramChannel
    from myassistant.channels.http_channel import HTTPChannel
    from myassistant.channels.whatsapp_channel import WhatsAppChannel
    from myassistant.channels.sms_channel import SMSChannel
    ...
    if "tray" in requested:
        import threading
        from myassistant.channels.tray_channel import TrayChannel
        tray = TrayChannel()
...
def providers():
    ...
    from myassistant.core.llm import list_providers, available_models, pick
    p = list_providers()
    print("== LLM Providers ==")
    for k, v in p.items():
        print(f"  [{'✓' if v else ' '}] {k}")
...
def doctor():
    ...
    from myassistant.core.doctor import run_doctor, format_report
    results = run_doctor()
...
def onboard():
    ...
    """
    from myassistant.core.onboarding_wizard import run_wizard
    run_wizard()
...
def mcp():
    ...
    """
    from myassistant.core.mcp_server import run_stdio_server
    registry.discover()
    run_stdio_server()
...
def plugin(
    ...
    """
    from myassistant.core.plugin_registry import (
